...
<<<<<<< HEAD
      <div className="text-center text-gray-800 space-y-3">
        <p className="text-2xl font-medium">
          Total Feedbacks:{" "}
          <span className="font-bold text-[#004f73]">{feedbacks.length}</span>
        </p>
        <p className="text-2xl font-medium">
          Total Page Views:{" "}
          <span className="font-bold text-[#004f73]">{totalViews}</span>
        </p>
      </div>

      {/* Top Reviews */}
      <div>
        <h3 className="text-3xl font-bold text-center text-[#004f73] mb-8">
          Top Reviews
        </h3>
        ...
        <div className="overflow-x-auto">
          <ul className="flex gap-6 pb-4 min-w-max px-2">
            {topReviews.map(({ id, name, comment, rating }) => (
              <li
                key={id}
                className="min-w-[280px] max-w-[320px] bg-gradient-to-tr from-white to-gray-100 border border-gray-200 rounded-2xl p-6 shadow-md hover:shadow-xl transition-shadow duration-300"
              >
